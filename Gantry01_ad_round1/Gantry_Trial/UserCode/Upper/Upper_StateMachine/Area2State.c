    ...
    if(1){      /*雷达状态正常*/
        if (stateflag == 0) 
        {
            if(weight_placement[1]==1){
                ...
                osDelay(300);
                Core_xy[1].gantry_t.position.y = 2000;  
        ...
        {
            if(weight_placement[1]==1){
                Core_xy[1].gantry_t.position.y = 3200;  
                TickType_t StartTick           = xTaskGetTickCount();
                initial_pos02[1]                 = Core_xy[1].Motor_Y->AxisData.AxisAngle_inDegree;
                _Bool isArray1                 = 0;
                float diff[1]                  = {0};
            ...
            if(weight_placement[1]==1){
                if(detect02_weight==0){
                     Core_xy[1].gantry_t.position.y =  Y_offset01 + angle_memory02weight;
                }else{
        ...
        /*前往木桩*/
            ...
            Core_xy[1].gantry_t.position.x = 17630.0;
            Core_xy[1].gantry_t.position.y = 2824.0;
            TickType_t StartTick           = xTaskGetTickCount();
            initial_pos02[0]                 = Core_xy[1].Motor_X->AxisData.AxisAngle_inDegree;
            initial_pos02[1]                 = Core_xy[1].Motor_Y->AxisData.AxisAngle_inDegree;
            ...
            TickType_t StartTick           = xTaskGetTickCount();
            initial_pos02[1]               = Core_xy[1].Motor_Y->AxisData.AxisAngle_inDegree; // 电机轴输出角度 单位 度°
            _Bool isArray1                 = 0;
            float diff[1]                  = {0};
            detect02ytree                  = 1;
            do {
        ...
        }else if (stateflag == 7)
        {
            ...
            stateflag = 8;
                ...
                Core_xy[1].gantry_t.position.x = 17600;
