...
#if (STM32F427xx)
DJI_t hDJI[8][2];       //定义16个电机的数据[i][0]为can1，[i][1]为can2
#endif
#if (STM32H723xx)
DJI_t hDJI[8][3];
#endif
 ...
 * @param	fdbData	反馈值
 */
void get_dji_offset(DJI_t *motor, uint8_t *fdbData)     //只是储存新的数据，但不更新到结构体中
{
...
/**
 * @brief	DJI电机CAN发送函数，电机ID 5/6/7/8     发送到电调由电调解码处理
 * @param	can	    CAN句柄(canx)
 * @param	cmx_iq	发送值，通常为最内环控制的输出值
 */
...
__weak void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    if (hcan->Instance == hcan1.Instance) {
        ...
        memcpy((uint8_t *)can1.rx_MSG.buffer, can1_rxdata, 8); // 保存CAN报文的内容         把&hcan1_rx接收到的信息都传给CAN_HANDLER结构体
