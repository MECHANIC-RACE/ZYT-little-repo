...
#define Y_maxvelocity  300 // 264.5
...
void Area2_State_Task(void *argument)
{
    inner_ring_flag = 1;
    osDelay(100);
    current_pos02[0]   = Lidar3.distance_aver;
    current_pos02[1]   = Lidar6.distance_aver;
    ...
    for (;;) {
        ...
        if (stateflag == 0) // 走到即将取第一个砝码之前
        {
            if (inner_ring_flag == 0) // 在里圈
            {
                ...
                Core_xy[1].gantry_t.position.x = 258.0; 
                Core_xy[1].gantry_t.position.y = 369.0;
                TickType_t StartTick           = xTaskGetTickCount();
                initial_pos02[0]                 = Lidar3.distance_aver;
                initial_pos02[1]                 = Lidar6.distance_aver;
                _Bool isArray1                 = 0;
                float diff[2]                  = {0};
                do {
                    TickType_t CurrentTick = xTaskGetTickCount();
                    float current_time     = (CurrentTick - StartTick) * 1.0 / 1000.0;
                    VelocityPlanning(initial_pos02[0], X_maxvelocity, 10, Core_xy[1].gantry_t.position.x, current_time, &(current_pos02[0]));
        ...
        } else if (stateflag == 1) // 放下气缸，打开磁铁，往前拖行，吸起砝码，提上气缸
        {
            ...
            if (inner_ring_flag == 0) { Core_xy[1].gantry_t.position.x = 94; } // 往前拖行一段
            else {
                Core_xy[1].gantry_t.position.x = 280;
            } // 往前拖行一段
        ...
        /*前往木桩*/
            ...
            Core_xy[1].gantry_t.position.x = 2395.0;
            Core_xy[1].gantry_t.position.y = 475.0;
            TickType_t StartTick           = xTaskGetTickCount();
            initial_pos02[0]                 = Lidar3.distance_aver;
            initial_pos02[1]                 = Lidar6.distance_aver;
            _Bool isArray2                 = 0;
