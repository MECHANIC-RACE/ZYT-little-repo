...
void Core_xy_State_Task(void *argument)
{
    ...
    for (;;) {
            ...
            if(stateflag==0)  //走到即将取第一个砝码之前
            {
                if (inner_ring_flag == 0) // 在外圈
                {
                    ...
                    do {
                        ...
                        VelocityPlanning(initial_pos[1],Y_maxvelocity, Y_Acceleration, Core_xy[0].gantry_t.position.y, current_time, &(current_pos[1]));
                    ...
                    do {
                        TickType_t CurrentTick = xTaskGetTickCount();
                        float current_time     = (CurrentTick - StartTick) * 1.0 / 1000.0;
                        VelocityPlanning(initial_pos[0], X_maxvelocity, 50, Core_xy[0].gantry_t.position.x, current_time, &(current_pos[0]));
                        VelocityPlanning(initial_pos[1], Y_maxvelocity, 50, Core_xy[0].gantry_t.position.y, current_time, &(current_pos[1]));
        ...
        do {
            TickType_t CurrentTick = xTaskGetTickCount();
            float current_time     = (CurrentTick - StartTick) * 1.0 / 1000.0;
            VelocityPlanning(initial_pos[0], X_maxvelocity, 40, Core_xy[0].gantry_t.position.x, current_time, &(current_pos[0]));
            VelocityPlanning(initial_pos[1], Y_maxvelocity, 50, Core_xy[0].gantry_t.position.y, current_time, &(current_pos[1]));
            diff[0] = fabs(Core_xy[0].gantry_t.position.x - current_pos[0]);
            diff[1] = fabs(Core_xy[0].gantry_t.position.y - current_pos[1]);
