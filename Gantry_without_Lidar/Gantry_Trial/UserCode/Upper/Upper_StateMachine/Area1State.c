 ...
 * @LastEditors: ZYT
 * @LastEditTime: 2024-06-12 00:59:13
        ...
        /*用于一个分区的路径规划*/
            ...
            if(stateflag==0)  //走到即将取第一个砝码之前
            {
                if (inner_ring_flag == 0) // 在li圈
                {
                    ...
                    Core_xy[0].gantry_t.position.y = -2777;
...
;
        ...
        {
        Core_xy[0].gantry_t.position.x = 7673;
        Core_xy[0].gantry_t.position.y = -3777;
        TickType_t StartTick           = xTaskGetTickCount();
        initial_pos01[0]                 = Core_xy[0].Motor_X->AxisData.AxisAngle_inDegree; // 电机轴输出角度 单位 度°
        initial_pos01[1]                 = Core_xy[0].Motor_Y->AxisData.AxisAngle_inDegree; // 电机轴输出角度 单位 度°
        _Bool isArray2 = 0;
        ...
        else if(stateflag==4)
        {
            ...
            Core_xy[0].gantry_t.position.x = 7400;
