...
void Upper_Servo_Task(void *argument)
{
    ...
    for (;;) {
        ...
        // positionServo_lidar(100, Core_xy[1].Motor_X, Lidar1);
        // positionServo_lidar(90, Core_xy[1].Motor_Y,Lidar2);

        //speedServo(5000, Core_xy[1].Motor_X);//正的时候往后走指靠近中心
        //speedServo(10000, Core_xy[1].Motor_Y);   //正的时候往左走指靠近中心
        //speedServo(5000, Core_xy[0].Motor_X);  //正的时候往前指靠近中心
        //speedServo(-5000, Core_xy[0].Motor_Y);  // 正的时候往右走指远离中心
